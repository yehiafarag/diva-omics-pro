/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package web.diva.client.somclust.view;

import com.google.gwt.user.client.Window;
import com.google.gwt.user.client.rpc.AsyncCallback;
import com.google.gwt.user.client.ui.HTML;
import com.google.gwt.user.client.ui.HorizontalPanel;
import com.google.gwt.user.client.ui.Image;
import com.google.gwt.user.client.ui.Label;
import com.google.gwt.user.client.ui.RootPanel;
import com.google.gwt.user.client.ui.VerticalPanel;
import com.smartgwt.client.types.Alignment;
import com.smartgwt.client.widgets.layout.HLayout;
import com.smartgwt.client.widgets.layout.VLayout;
import web.diva.client.GreetingServiceAsync;
import web.diva.client.selectionmanager.ModularizedListener;
import web.diva.client.selectionmanager.Selection;
import web.diva.client.selectionmanager.SelectionManager;
import web.diva.shared.beans.SomClustTreeSelectionUpdate;
import web.diva.shared.beans.SomClusteringResult;

/**
 *
 * @author Yehia Farag
 */
public class UpdatedSomClustView extends ModularizedListener {

    @Override
    public String toString() {
        return "SomClustView"; //To change body of generated methods, choose Tools | Templates.
    }

    private SelectionManager selectionManager;
    private  String defaultSideTreeImgURL;
    private String defaultTopTreeImgURL;
    private boolean update = true;
    private boolean clustColumn=true;

    private final GreetingServiceAsync greetingService;

    private  TreeImg sideTreeImg;
    private TreeImg upperTreeImg;
    private  HeatmapImg heatMapImg;
    private  Image scaleImg;
    private final HLayout clusterLayout = new HLayout();
    private final VerticalPanel clusteringPanel = new VerticalPanel();
    private final VLayout leftSideLayout = new VLayout();
    private final VLayout rightSideLayout = new VLayout();
    
    
    
     private final VLayout mainThumbPCALayout;
    private final HTML tooltipLabel = new HTML();
    private HorizontalPanel tooltipViewPortLayout;
    private  Image thumbChart; //maxmizePlotImage,
//    private final PopupPanel pcaPopup;
    private int sideTreeWidth,upperTreeWidth,sideTreeHeight;
    private boolean isMaxmizable = false;

    public UpdatedSomClustView(SomClusteringResult somClusteringResults, SelectionManager selectionManager, GreetingServiceAsync greetingService, int width, int height, boolean clustColumn) {
        this.greetingService = greetingService;
        this.defaultSideTreeImgURL = somClusteringResults.getSideTreeImgUrl();
        mainThumbPCALayout = new VLayout();
        mainThumbPCALayout.setStyleName("somclustering");
        mainThumbPCALayout.setHeight("89%");
        mainThumbPCALayout.setWidth("29%");
        mainThumbPCALayout.setMargin(2);
        mainThumbPCALayout.setMembersMargin(0);
        
        
        HorizontalPanel topLayout = new HorizontalPanel();
        mainThumbPCALayout.addMember(topLayout);
        topLayout.setWidth("100%");
        topLayout.setHeight("18px");
        topLayout.setStyleName("whiteLayout");
        Label title = new Label("hierarchical clustering");
        title.setStyleName("labelheader");
        topLayout.add(title);
        title.setWidth("90%");
        topLayout.setCellHorizontalAlignment(title, HorizontalPanel.ALIGN_LEFT);
        Label maxmizeBtn = new Label();
        maxmizeBtn.addStyleName("maxmize");
        maxmizeBtn.setHeight("16px");
        maxmizeBtn.setWidth("16px");
        topLayout.add(maxmizeBtn);
        topLayout.setCellHorizontalAlignment(maxmizeBtn, HorizontalPanel.ALIGN_RIGHT);
         int topLeftHight = 0;
        if (clustColumn) {
            topLeftHight = 70;
        }

        mainThumbPCALayout.addMember(clusterLayout);

        clusterLayout.setWidth("100%");
        clusterLayout.setHeight("100%");
        clusterLayout.setAlign(Alignment.RIGHT);

        clusterLayout.addMembers(leftSideLayout, rightSideLayout);

        leftSideLayout.setWidth("60%");
        leftSideLayout.setHeight("100%");
//        leftSideLayout.setStyleName("layouttest1");

        rightSideLayout.setWidth("40%");
        rightSideLayout.setHeight("100%");
//        rightSideLayout.setStyleName("layouttest2");
        
//        int clusteringHight = topLeftHight+somClusteringResults.getSideTreeHeight()+50;
//        int clusteringWidth = somClusteringResults.getSideTreeWidth()+somClusteringResults.getTopTreeWidth();
//        
//        if(clusteringHight > RootPanel.get().getOffsetHeight()|| clusteringWidth > RootPanel.get("templatemo_right").getOffsetWidth())
//            isMaxmizable = true;
        
        
        
//        
//       
//         
//        rightSideLayout.setAlign(Alignment.CENTER);
//        sideTreeHeight = leftSideLayout.getHeight();
//        upperTreeWidth = rightSideLayout.getWidth();
//        
//        
//        
//        if(sideTreeHeight > somClusteringResults.getSideTreeHeight())
//            sideTreeHeight = somClusteringResults.getSideTreeHeight();
//        else
//            isMaxmizable = true;
//        
//         if(upperTreeWidth > somClusteringResults.getSideTreeWidth())
//            upperTreeWidth = somClusteringResults.getSideTreeWidth();
//         else
//            isMaxmizable = true;
//        
//         
//         rightSideLayout.setWidth(upperTreeWidth);
//      
//        sideTreeImg = new TreeImg(somClusteringResults.getSideTreeImgUrl(), somClusteringResults.getRowNode(), 2);//"data:sideTreeImg/png;base64,");
//        sideTreeImg.setWidth(sideTreeWidth+"px");
////        clustColumn = false;
//        this.clustColumn = clustColumn;
//       
//
//        heatMapImg = new HeatmapImg(somClusteringResults.getHeatMapImgUrl(), somClusteringResults.getRowNames(), somClusteringResults.getColNames(), somClusteringResults.getValues());
//        rightSideLayout.addMember(clusterLayout);
//        heatMapImg.setWidth(upperTreeWidth+"px");
////        heatMapImg.setWidth("40%");
//        scaleImg = new Image(somClusteringResults.getScaleImgUrl());
//        scaleImg.setWidth(upperTreeWidth+"px");
////        scaleImg.setWidth("40%");
//        if (clustColumn) {
//            defaultTopTreeImgURL = somClusteringResults.getUpperTreeImgUrl();
//            upperTreeImg = new TreeImg(somClusteringResults.getUpperTreeImgUrl(), somClusteringResults.getColNode(), 1);
//            rightSideLayout.addMember(upperTreeImg);
//
//            rightSideLayout.addMember(heatMapImg);
//            rightSideLayout.addMember(scaleImg);
//
//            upperTreeImg.setHeight("70px");
//            upperTreeImg.setWidth(upperTreeWidth+"px");
//
//           
////            Label spacer = new Label("");
////            spacer.setHeight(upperTreeImg.getHeight() + "px");
////            spacer.setWidth(sideTreeImg.getWidth() + "px");
//////            leftSideLayout.addMember(spacer);
////            upperTreeImg.addClickHandler(new ClickHandler() {
////                @Override
////                public void onClick(ClickEvent event) {
////                    int y = (int) (event.getY());
////                    int x = ((int) (event.getX()));
////                    if (upperTreeImg.isSelectedNode()) {
////                        updateUpperTreeSelection(x, y);
////                    }
////                }
////            });
////            rightSideLayout.setWidth(upperTreeImg.getWidth());
////
////            rightSideLayout.addMember(upperTreeImg);
//        } else {
//            rightSideLayout.addMember(heatMapImg);
//            rightSideLayout.addMember(heatMapImg);
//        }
//         if(rightSideLayout.getHeight() < (sideTreeHeight+50+topLeftHight))
//             isMaxmizable = true;
////        scaleImg.setWidth("100%");
//        scaleImg.setHeight("20px");
//        heatMapImg.setHeight("100%");
////        heatMapImg.setWidth("100%");
//
//
////        spacer.addStyleName("magnifying");
//        Label info1 = new Label("Distance metrics : " + somClusteringResults.getDistanceMeasure());
//        info1.setWidth(leftSideLayout.getWidth() + "px");
//        info1.setHeight("20px");
//        info1.setStyleName("info");
//        Label info2 = new Label("Linkage : " + somClusteringResults.getLinkage());
//        info2.setWidth(leftSideLayout.getWidth() + "px");
//        info2.setHeight("20px");
//        info2.setStyleName("info");
//
//        //init top layout
//        
//
////        leftSideLayout.addMember(sideTreeImg);
//        sideTreeImg.setWidth("100%");
////        leftSideLayout.addMember(info1);
////        leftSideLayout.addMember(info2);
//
//       
//
////        spacer.addClickHandler(new ClickHandler() {
////
//            @Override
//            public void onClick(ClickEvent event) {
//                final Window winModal = new Window();
//                winModal.setStyleName("testo");
//                winModal.setWidth("50%");
//                winModal.setHeight("70%");
//                winModal.setTitle("Hierarchical Clustering Panel");
//                winModal.setShowMinimizeButton(false);
//                winModal.setShowCloseButton(true);
//                winModal.setOverflow(Overflow.SCROLL);
//                winModal.setIsModal(true);
//                winModal.setShowModalMask(true);
//                winModal.centerInPage();
//                winModal.addCloseClickHandler(new CloseClickHandler() {
//                    @Override
//                    public void onCloseClick(CloseClickEvent event) {
//                        winModal.close();
//                        clusteringPanel.add(clusterLayout);
//
//                    }
//                });
//                winModal.show();
//            }
//        });
        //init middle layout   
//        rightSideLayout.addMember(heatMapImg);
//        rightSideLayout.addMember(scaleImg);
       
//        clusteringPanel.add(clusterLayout);
//        sideTreeImg.addClickHandler(new ClickHandler() {
//            @Override
//            public void onClick(ClickEvent event) {
//                int y = (int) (event.getY());
//                int x = ((int) (event.getX()));
//                if (sideTreeImg.isSelectedNode()) {
//                    updateSideTreeSelection(x, y);
//                }
//
//            }
//
//        });
//
//        this.selectionManager = selectionManager;
//        this.selectionManager.addSelectionChangeListener(UpdatedSomClustView.this);
//        somClusteringResults = null;
    }

    public VLayout getSomclusteringLayout() {
        return mainThumbPCALayout;
    }

    @Override
    public void selectionChanged(Selection.TYPE type) {
        if (type == Selection.TYPE.OF_ROWS) {
            Selection sel = selectionManager.getSelectedRows();
            if (sel != null) {
                int[] selectedRows = sel.getMembers();
                if (selectedRows != null && update) {
                    clearRowSelection();
                }
            }
        } else {
            Selection sel = selectionManager.getSelectedColumns();
            if (sel != null) {
                int[] selectedColumns = sel.getMembers();
                if (selectedColumns != null && update) {
                    clearColSelection();
                }
            }

        }
    }

    public void clearRowSelection() {
        sideTreeImg.setUrl(defaultSideTreeImgURL);
        
    }
    public void clearColSelection(){
    if(clustColumn)
            upperTreeImg.setUrl(defaultTopTreeImgURL);    
    }

    @Override
    public void remove() {
        selectionManager = null;
    }

    private void updateSideTreeSelection(int x, int y) {
         selectionManager.busyTask(true,false);
        greetingService.updateSideTree(x, y, 200, (500 - 25.0), new AsyncCallback<SomClustTreeSelectionUpdate>() {
            @Override
            public void onFailure(Throwable caught) {
                 selectionManager.busyTask(false,false);
            }

            @Override
            public void onSuccess(SomClustTreeSelectionUpdate result) {
                sideTreeImg.setUrl(result.getTreeImgUrl());
                if (result.getSelectedIndices() != null) {
                    update = false;
                    Selection selection = new Selection(Selection.TYPE.OF_ROWS, result.getSelectedIndices());
                    selectionManager.setSelectedRows(selection);
                    update = true;
                }
                selectionManager.busyTask(false,false);
            }
        });
    }

    private void updateUpperTreeSelection(int x, int y) {
        selectionManager.busyTask(true,false);
        greetingService.updateUpperTree(x, y, 70, (500 - 25.0), new AsyncCallback<SomClustTreeSelectionUpdate>() {
            @Override
            public void onFailure(Throwable caught) {
                selectionManager.busyTask(false,false);
            }

            @Override
            public void onSuccess(SomClustTreeSelectionUpdate result) {
                upperTreeImg.setUrl(result.getTreeImgUrl());//          
                if (result.getSelectedIndices() != null) {
                    update = false;
                    Selection selection = new Selection(Selection.TYPE.OF_COLUMNS, result.getSelectedIndices());
                    selectionManager.setSelectedColumns(selection);
                    update = true;
                }
                selectionManager.busyTask(false,false);
            }
        });
    }

}
